...
const StudentLayout = ({ children, activeTab }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] flex relative">
      <Sidebar activeTab={activeTab} mobileOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      ...
      >
        {isAIOpen ? <X size={28} strokeWidth={2.5} className="sm:w-8 sm:h-8" /> : <img src="/Picture1.png" alt="AI Mentor Logo" className="w-full h-full object-cover" />}
      </button>
    </div>
  );
};
