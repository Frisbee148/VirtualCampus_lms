...
const AttendanceTable = () => {
  ...
  const addStudent = () => {
    ...
    setStudents((prev) => [
      ...prev,
      { id: newId, name: `Student ${newId}`, attendance: {} },
    ]);
  };
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="relative">
            <button
              ...
              className="bg-[#4E545C] text-white px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium flex items-center gap-2"
            >
          ...
          <thead>
            <tr className="bg-[#4E545C] text-white">
              ...
              {dates.map((date) => (
                <th
                  key={date}
                  className="text-left px-2 sm:px-4 py-2 sm:py-3 font-semibold min-w-[60px] sm:min-w-[80px]"
                >
                  {typeof date === "number"
                    ? `${selectedMonth ? selectedMonth.slice(0, 3) : ""} ${date}`
                    : date}
                </th>
              ))}
            </tr>
              ...
              {dates.map((date) => (
                ...
                >
                  <span className="text-xs text-gray-600">
                    {typeof date === "number"
                      ? `${selectedMonth ? selectedMonth.slice(0, 3) : "Date"} ${date}`
                      : date}
                  </span>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
